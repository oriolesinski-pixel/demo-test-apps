(function(root, factory) {
  ...
  class AnalyticsTracker {
    constructor() {
      ...
      this.config = {
        appKey: 'demo-test-apps-2025-10-14-byl4nfcpzgp',
    ...
    initAutoTracking() {
      console.log('[demo-test-apps-2025-10-14-byl4nfcpzgp] 🤖 AI-Enhanced Analytics initialized');
      console.log('[demo-test-apps-2025-10-14-byl4nfcpzgp] 📊 Tracking 0 discovered components');
      console.log('[demo-test-apps-2025-10-14-byl4nfcpzgp] 🔑 User ID:', this.userId);
      console.log('[demo-test-apps-2025-10-14-byl4nfcpzgp] 📍 Session ID:', this.sessionId);
    ...
    extractContext(element, componentInfo) {
      ...
      for (const fieldConfig of componentInfo.context_collection.fields) {
        try {
          ...
          console.debug(`[demo-test-apps-2025-10-14-byl4nfcpzgp] Context extraction failed for ${fieldConfig.field_name}`, error);
        }
      }
    ...
    flush() {
      ...
      .then(response => {
        if (!response.ok) {
          console.error('[demo-test-apps-2025-10-14-byl4nfcpzgp] Analytics flush failed:', response.status);
          // Re-add to queue and persist on failure
          this.eventQueue.unshift(...batch);
          this.saveQueueToStorage();
        } else {
          console.debug('[demo-test-apps-2025-10-14-byl4nfcpzgp] Flushed', batch.length, 'events');
        }
      })
      .catch(err => {
        console.error('[demo-test-apps-2025-10-14-byl4nfcpzgp] Analytics flush error:', err);
        // Re-add to queue and persist on failure
        this.eventQueue.unshift(...batch);
        this.saveQueueToStorage();
      });
  ...
  if (typeof window !== 'undefined' && !window.analytics) {
    ...
    console.log('[demo-test-apps-2025-10-14-byl4nfcpzgp] ✅ AI-Enhanced Analytics tracker with new event schema initialized');
  }

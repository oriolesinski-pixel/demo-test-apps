...
export function ProjectForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check workspace plan and project count
      ...
      // Check if at limit on free plan
      if (plan === 'free' && (projectCount || 0) >= 3) {
        setCurrentProjectCount(projectCount || 0)
        setShowPaywall(true)
        setLoading(false)
        
        // Log paywall encounter
        await supabase.from('activity_log').insert({
          workspace_id: workspaceMember.workspace_id,
          user_id: user.id,
          action: 'paywall_encountered',
          entity_type: 'project',
          entity_id: workspaceMember.workspace_id,
          metadata: { 
            context: 'project_limit',

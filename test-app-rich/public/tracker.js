(function(root, factory) {
  ...
  class AnalyticsTracker {
    constructor() {
      ...
      this.config = {
        appKey: 'demo-test-apps-2025-10-06-vcuan82b29',
      ...
      this.componentDetectors = [
        {
            name: 'LoginButton',
            type: 'button',
            selectors: [".appearance-none.relative.block.w-full.px-3.py-2.border.border-gray-300.placeholder-gray-500.text-gray-900.rounded-t-md.focus:outline-none.focus:ring-blue-500.focus:border-blue-500.focus:z-10.sm:text-sm"],
...

        {
            ...
            contextCollection: {"search_parents":["div","nav"]}
        },

        {
            name: 'TestCredentialsButton',
            type: 'button',
            selectors: ["onClick={fillTestCredentials}"],
            purpose: 'Fill in test credentials for login',
            contextNeeded: undefined,
...

        {
            ...
            type: 'button',
            selectors: ["onClick={() => removeFromCart(item.id)}"],
            purpose: 'Remove item from cart',
            contextNeeded: ["productId"],
            contextCollection: {"search_parents":[".product-card","[data-product]"],"data_attributes":["data-product-id"]}
        },

        {
            name: 'QuantitySelector',
            type: 'selector',
            selectors: ["onClick={() => updateQuantity(item.id, item.quantity - 1)}","onClick={() => updateQuantity(item.id, item.quantity + 1)}"],
            purpose: 'Adjust quantity of cart item',
            contextNeeded: ["productId","quantity"],
            contextCollection: {"search_parents":[".product-card","[data-product]"],"data_attributes":["data-product-id","data-quantity"]}
        },

        {
            name: 'ClearCartButton',
            type: 'button',
            selectors: ["onClick={clearCart}"],
...

        {
            ...
            type: 'form',
            selectors: ["onSubmit={handleSubmit}"],
            purpose: 'Provide shipping information for checkout',
...

        {
            name: 'SlideIndicator',
            type: 'selector',
            selectors: ["onClick={() => goToSlide(index)}"],
            purpose: 'Navigate to a specific carousel slide',
            contextNeeded: ["slideIndex"],
            contextCollection: {"search_parents":[".carousel"],"data_attributes":["data-slide-index"]}
        },

        {
            name: 'MobileMenuToggle',
            type: 'toggle',
            selectors: ["onClick={() => setMobileMenuOpen(!mobileMenuOpen)}"],
            purpose: 'Toggle mobile menu visibility',
            contextNeeded: undefined,
            contextCollection: {"search_parents":["header","nav"]}
        },

        {
            name: 'WishlistToggle',
            type: 'toggle',
            selectors: ["onClick={() => toggleWishlist(item.id)}"],
...

        {
            name: 'CartCountBadge',
            type: 'custom',
            selectors: [".absolute.-top-2.-right-2.bg-red-500.text-white.text-xs.rounded-full.w-5.h-5.flex.items-center.justify-center"],
            purpose: 'Display the number of items in the cart',
            contextNeeded: ["itemCount"],
            contextCollection: {"search_parents":["header","nav"],"data_attributes":["data-item-count"]}
        }
      ];
    ...
    initAutoTracking() {
      console.log('🤖 AI-Enhanced Analytics initialized for demo-test-apps-2025-10-06-vcuan82b29');
